...
def test_stack_creation_deletion(render):
    ...
    r=render.run(renderapi.stack.create_stack,test_stack,force_resolution=True)
...
def test_simple_import(render,simpletilespec,tmpdir):
    ...
    r=render.run(renderapi.stack.create_stack,'test_insert',force_resolution=True)
    render.run(renderapi.client.import_single_json_file,'test_insert',str(tfile))
    r=render.run(renderapi.stack.set_stack_state,'test_insert','COMPLETE')
    assert (r.status_code == 201)
    ts_out = render.run(renderapi.tilespec.get_tile_spec,'test_insert',simpletilespec.tileId)
    assert (ts_out.z == simpletilespec.z)
    render.run(renderapi.stack.delete_stack,'test_insert')

def test_simple_import_with_transforms(render,render_example_tilespec_and_transforms,tmpdir):
    ...
    r=render.run(renderapi.stack.create_stack,'test_insert_tform',force_resolution=True)
    render.run(renderapi.client.import_single_json_file,'test_insert_tform',str(tilespecfile),transformFile=str(transformfile))
    r=render.run(renderapi.stack.set_stack_state,'test_insert_tform','COMPLETE')
    assert (r.status_code == 201)
    ts_out = render.run(renderapi.tilespec.get_tile_spec,'test_insert_tform',tilespecs[0].tileId)
...
def test_import_tilespecs(render,simpletilespec):
    ...
    assert (r.status_code == 201)
    ts_out = render.run(renderapi.tilespec.get_tile_spec,stack,simpletilespec.tileId)
    assert (ts_out.z == simpletilespec.z)
...
def test_stack_bounds(render,teststack):
    ...
    expected_bounds={u'maxZ': 3408.0, u'maxX': 5176.0, u'maxY': 5319.0, u'minX': 232.0, u'minY': 17.0, u'minZ': 3407.0}
...
def test_z_bounds(render,teststack,render_example_tilespec_and_transforms):
    ...
    zbounds = render.run(renderapi.stack.get_bounds_from_z,teststack,tilespecs[0].z)
    expected_bounds = {u'maxZ': 3407.0, u'maxX': 5009.0, u'maxY': 4395.0, u'minX': 232.0, u'minY': 17.0, u'minZ': 3407.0}
    assert(zbounds==expected_bounds)

def test_get_section_z(render,teststack):
    #check getting section Z
    z = render.run(renderapi.stack.get_section_z_value,teststack,"3407.0")
    assert(z==3407)
    z = render.run(renderapi.stack.get_z_value_for_section,teststack,"3407.0")
    assert(z==3407)
...
def test_bb_image(render,teststack):
    ...
    for format in formats:
        data = render.run(renderapi.image.get_bb_image,teststack,
                                                       z,x,y,width,height,
                                                       scale=.25,
                                                       img_format = format)
        dr = data.ravel()
...
def test_tile_image(render,teststack,render_example_tilespec_and_transforms):
    (tilespecs,tforms)=render_example_tilespec_and_transforms
    format = 'png'
    data=render.run(renderapi.image.get_tile_image_data,teststack,tilespecs[0].tileId)
...
def fail_image_get(render,teststack,render_example_tilespec_and_transforms):
    with pytest.raises(KeyError):
        render.run(renderapi.image.get_tile_image_data,teststack,'test',img_format='JUNK')
