...
def assert_estimation_sum(tforml):
    src_pts = 3*np.random.random((10,2))
    tform_sum = renderapi.transform.estimate_transformsum(tforml,src=src_pts,order=1)
...
def test_tranform_nested_estimate():
    ...
    assert_estimation_sum(tform_metalist)
    assert_estimation_sum(tform_list_list)
    assert_estimation_sum([tform_list,tform_list2])
